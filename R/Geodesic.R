...
#'
#' With this function it is possible to convert from Cartesian coordinate to
#' Geographic coordinate and obtain the decimal precision that you assign.
#'
...
#'
#' @return data.frame with the data in the following order: "Pt", "Lat", "Lon",
#' "H".
#' @export
#'
...
#  Ellipsoids <- NULL
  ...
  lat <- as.numeric(atan((z+as.numeric(Ellipsoids[a,6])*
                            as.numeric(Ellipsoids[a,8])*
                            sin(atan((as.numeric(Ellipsoids[a,3])*z)/
                                       (as.numeric(Ellipsoids[a,6])*
                                          sqrt(x^2+y^2))))^3)/
                           ((sqrt(x^2+y^2))-as.numeric(Ellipsoids[a,3])*
                              as.numeric(Ellipsoids[a,7])*
  ...
  values <- tibble::as_tibble(as.data.frame(
    cbind(XYZ_df[,1],round(lat*180/pi, digits), round(lon*180/pi, digits),
          round(H, digits))))
