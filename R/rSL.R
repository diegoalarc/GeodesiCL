...
#'
#' @param x Selection of Ellipsoid to work between 1 = 'PSAD-56', 2 = 'SAD-69',
#' 3 = 'WGS-84',	4 ='GRS-80 (SIRGAS)'.
...
rSL <- function(x, pto_lat, digits = 4){
  ...
  r <- as.numeric(Ellipsoids[x,3])/sqrt(1 - as.numeric(Ellipsoids[x,7])*
                                          sin(y)^2)*cos(y)
  S <- (as.numeric(Ellipsoids[x,3])*(1 - as.numeric(Ellipsoids[x,7]))/
          (1 - as.numeric(Ellipsoids[x,7])*sin(y)^2)^(3/2))*y
  L <- (as.numeric(Ellipsoids[x,3])/sqrt(1 - as.numeric(Ellipsoids[x,7])*
                                           sin(y)^2)*cos(y))*y
  values <- tibble::as_tibble(data.frame(pto_lat[,1], round(r, digits),
                                         round(S, digits), round(L, digits)))
